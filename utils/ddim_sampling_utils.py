...
@torch.no_grad()
def short_cut_infer(latents_x0, actions, eps_i, model, vae):
    denoise_timesteps_list=[1]
    ...
    for i, denoise_timesteps in enumerate(denoise_timesteps_list):
        ...
        for ti in range(denoise_timesteps):
            ...
            t_vector = torch.full((eps_i.shape[0],), t).to(latents_x0.device)
            # t_vector = torch.full((eps.shape[0],), t)
            dt_base = torch.ones_like(t_vector).to(latents_x0.device) * math.log2(denoise_timesteps)
            # dt_base = torch.ones_like(t_vector) * math.log2(denoise_timesteps)


            with torch.no_grad():
                v = model(torch.cat([latents_x0,x],dim=2), t_vector, actions, dt_base, f1)[:,:,f1:,:,:]
            ...
            if denoise_timesteps <= 8 or ti % (denoise_timesteps//8) ==0 or ti == denoise_timesteps-1:
                with torch.no_grad():
                    ...
                    decoded = vae.decode(rearrange(torch.cat([latents_x0,x],dim=2), 'b c f h w -> (b f) c h w')/0.18215)[0]
                    decoded = rearrange(decoded, '(b f) c h w -> b c f h w', f=f1+f2)
        ...
        print("denoise_timesteps:",denoise_timesteps,"time:",time.time()-t0)
        ...
        #     # img=img[:,:1,:,:]
        #     img=rearrange(img, 'c f h w -> c h (f w)')
        #     img = img.permute(1,2,0)
        #     return img
        # fig, axs = plt.subplots(8, all_x.shape[1], figsize=(30,30))
        # for t in range(min(8, all_x.shape[0])):
        #     for j in range(all_x.shape[1]):        
        #         axs[t, j].imshow(process_img(all_x[t, j]), vmin=0, vmax=1)

        # fig.savefig(f"denoise_timesteps-{denoise_timesteps}.png")

        return all_x

@torch.no_grad()
def short_cut_infer_onestep(latents_x0, actions, eps_i, model, vae, visualize, image_id=None):
    # print("========================start================================")
    ...
    with torch.no_grad():
        if visualize:
            ...
            decoded = vae.decode(rearrange(x[:,:,image_id:image_id+1,:,:], 'b c f h w -> (b f) c h w')/0.18215)[0]
            decoded = rearrange(decoded, '(b f) c h w -> b c f h w', f=1)
