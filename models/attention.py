...
class SpatialTransformer3D(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0.0, context_dim=None):
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                TransformerBlock3D(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim)
...
class TemporalTransformer3D(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0.0, context_dim=None, temporal=False, causal = False):
        super().__init__()
        self.n_heads = n_heads
        self.d_head = d_head
        self.in_channels = in_channels
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                TransformerBlock3D_Temporal(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim, temporal=temporal, causal = causal)
                for d in range(depth)
            ]
        )
        self.proj_out = InflatedConv3d(inner_dim, in_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, context=None, cond_frame = 0, return_attn = False):
        ...
        for block in self.transformer_blocks:
            if return_attn:
                x,attn = block(x, cond_frame = cond_frame, return_attn = return_attn)
            else:
                x = block(x, cond_frame = cond_frame)
        x = x.reshape(b, f, h, w, c).permute(0, 4, 1, 2, 3).contiguous()
        x = self.proj_out(x) + x_in
        if return_attn:
            return Transformer3DModelOutput(sample=x)['sample'],attn
...
class TransformerBlock3D_Temporal(nn.Module):
    ...
    def forward(self, x, cond_frame = 0):
        b, c, f, h, w = x.shape
        if self.temporal:
        ...
        if self.with_adapter:
            # print("x.shape after temp attn: ",x.shape)
            x = self.my_module_1(x.reshape(b, f, h, w, c)).reshape(b, f * h * w, c)
...
class TransformerBlock3D(nn.Module):
    ...
    def forward(self, x, context=None, cond_frame = 0, return_attn = False):
        ...
        if self.with_adapter:
            x = rearrange(x, "(b f) (h w) c -> b f h w c", b=b, f=f, h=h, w=w)
